var tools = {
	...
	parents:function (element,selector,parent){
		/*
			element： 目标元素
			selector：选择器(className /id /tagName)
		*/

		var first = selector.charAt();	//找到选择器的第一个字符

		if( first === "#" ){			
			selector = selector.slice(1); 
			while(element.nodeType != 9 && element.id != selector){  //当前这个元素不是document，并且id不等于selector
				element = element.parentNode;
			}
		}else if(first === "."){
			selector = selector.slice(1); 
			while(element.nodeType != 9 && !tools.hasClass(element,selector)){  
				element = element.parentNode;
			}
		}else {
			while(element.nodeType != 9 && element.nodeName.toLowerCase() != selector){  
				element = element.parentNode;
			}
		}
